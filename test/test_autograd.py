...
import trytorch.autograd as autograd
import trytorch.ops as ndl
...
def test_topo_sort():
    ...
    topo_order = np.array([x.numpy() for x in autograd.find_topo_sort([c1])])
    ...
    topo_order = [x.numpy() for x in autograd.find_topo_sort([c1])]
    ...
    topo_order = np.array([x.numpy() for x in autograd.find_topo_sort([e])])
